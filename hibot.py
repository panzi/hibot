...
GREETING = \
	r"(?:(?:also,|quick)?\s+)?(?:ay|oi|ole|ha*i+|hey+|heys+|hia+|hi-?ya+|high|heya+|heyo+|h[eau]llo+|hola+|howdy+doo+dy+|greetings+|howdy+|welcome(?:\s+back)?|(?:what)?'?s\s*up(?:\s*dog+)?|what\s*up(?:\s*dog+)?|howdy-?do+|yo+|wh?add?\s*up(?:\s*dog+)?|yuhu+|good\s*day|'?g\s*day)(?:\s+(?:to|there))?"
